...
class Routes{
	...
	*	@example 'controller' path to a controller that uses default action 'index'
	...
	*/
	public static function getRoute($route, $param = array()){
		$route = explode('#', $route);
		$routeName = $route[0];
		$routeAction = isset($route[1]) ? $route[1] : null;
		...
		if( (is_string($routes) && $routes === 'all' ) && ($routeAction === null || !array_key_exists($routeAction, self::$standardRoutes)) ){
			throw new \Exception('Route does not exsist: ' . $controller . '#' . $routeAction);
		}
	...
	public static function match($controller, $action){
		if(!array_key_exists($controller, self::$routes)){
			throw new \Exception('No routes for controller: ' . $controller);
		}
